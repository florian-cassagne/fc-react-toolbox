export const animationDefault = 'animation-all duration-200 '
export const colorPrimary = 'bg-orange-900 hover:bg-orange-800 '

export const colorSecondary = 'bg-neutral-900 hover:bg-neutral-800 '
export const colorSecondaryNoHover = 'bg-neutral-800 '

export const colorImportant =
  'bg-red-600 hover:bg-red-500 border border-red-300 '

export const shadowDefault = 'shadow-[0_0_5px_3px_rgba(255,255,255,0.3)] '
